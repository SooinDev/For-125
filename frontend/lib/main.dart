...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Future<void> _checkServerConnection() async {
    ...
    try {
      ...
      print('[INFO] 서버 응답 코드: ${response.statusCode}');
      print('[DEBUG] 서버 응답 내용: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[SUCCESS] 파싱된 데이터 - status: $status, liveImageUrl: $liveImageUrl, channelId: $channelId, concurrentUserCount: $concurrentUserCount');
        ...
        print('[ERROR] 404 에러: /api/stream/live-status 엔드포인트를 찾을 수 없습니다');
        setState(() {
          _statusMessage = '🔍 서버에 /api/stream/live-status 엔드포인트가 없어요 (404)';
        });
      } else {
        print('[ERROR] HTTP 에러: ${response.statusCode}');
        setState(() {
      ...
      setState(() {
        if (e.toString().contains('Connection refused') ||
          ...
          _statusMessage = '🚫 서버가 꺼져있어요 (연결 거부)';
        } else if (e.toString().contains('TimeoutException')) {
          _statusMessage = '⏰ 서버 응답 시간 초과';
        } else {
          _statusMessage = '🌸 네트워크 오류: ${e.toString().split(':').first} 🌸';
        }
      });
    }
  }
  ...
  Future<void> _launchChzzkUrl() async {
    ...
    final chzzkLiveUrl = Uri.parse('https://chzzk.naver.com/live/$_channelId');
    if (!await launchUrl(chzzkLiveUrl, mode: LaunchMode.externalApplication)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('치지직 방송을 열 수 없습니다: $chzzkLiveUrl')),
        );
      }
    }
  }
