  ...
  public List<ReplayVO> getReplays() {
    ...
    try {
      ...
      if (response != null && response.getContent() != null && response.getContent().getData() != null) {
        ...
        java.util.List<ReplayVO> filteredReplays = response.getContent().getData().stream()
                .filter(replay -> {
                  if (replay.getPublishDateAt() == null || replay.getPublishDateAt().isEmpty()) {
                    return false;
                  }
                  try {
                    // publishDateAt이 "2024-11-04 16:04:32" 형식
                    java.time.LocalDateTime publishDate = java.time.LocalDateTime.parse(
                            replay.getPublishDateAt(),
                            java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")
                    );
                    return publishDate.isAfter(thirtyDaysAgo);
                  } catch (Exception e) {
                    System.err.println("[WARN] 날짜 파싱 실패: " + replay.getPublishDateAt());
                    return false;
                  }
                })
                .collect(java.util.stream.Collectors.toList());

        System.out.println("[INFO] 최근 30일 영상 필터링 결과: " + filteredReplays.size() + "개");
